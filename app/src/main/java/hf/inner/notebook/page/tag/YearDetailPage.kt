...
import hf.inner.notebook.utils.toMM
...
fun YearDetailPage(year: String, navController: NavHostController) {
    ...
    val pageTitle = remember { mutableStateOf(year) }
    ...
    fun YearBottomSheet(show: Boolean, onDismissRequest: () -> Unit) {
        val monthList = arrayListOf("01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12")
    ...
    RYScaffold(
        ...
        actions = {
            IconButton(onClick = {
                ...
                Icon(
                    imageVector = Icons.Outlined.FilterList, contentDescription = stringResource(R.string.sort)
                )
            }
        }
    ) {
